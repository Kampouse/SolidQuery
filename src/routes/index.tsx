    ...
    return (
        <div class="header col-span-12 rounded-lg   bg-inner-blue   flex flex-row mb-4">
            <For each={props.fields}>
                {(field) => (
                    <button class="bg-white text-black rounded-lg   border border-inner-blue  p-4 px-8"> <A href={ishome("/" + field)}> {field} </A> </button>
                )}
            </For>
        </div>
...
const Fields = ["Home", "DarkMode", "ColorsPalette", "Modal", "Form", "DogFrame"]
